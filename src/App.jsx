...
import { Aperture, X, Settings as SettingsIcon, Sparkles } from 'lucide-react';
        ...
        >
          <div className='bg-glass-medium backdrop-blur-2xl border border-black/5 rounded-full px-8 py-4 shadow-glass-sm flex items-center gap-12 max-w-2xl w-full justify-between hover:border-black/10 transition-colors duration-500'>
            ...
            <nav className='flex items-center space-x-8 text-[11px] font-bold tracking-[0.15em] uppercase text-dark-text-secondary'>
              <button
                onClick={() => setView('home')}
                className={`hover:text-black transition-all duration-300 py-1 relative ${
                  view === 'home' ? 'text-black' : ''
                }`}
              >
                Home
                {view === 'home' && (
                  <motion.div
                    layoutId='nav-glow'
                    className='absolute -bottom-1 left-0 right-0 h-px bg-black/20 shadow-glow'
                  />
                )}
              </button>
              <button
        ...
        {view === 'gallery' && (
          ...
          >
            <div className='w-full max-w-6xl px-4 mb-16 text-center space-y-4'>
              <h2 className='text-5xl font-thin tracking-tighter text-white'>
                Collection
              </h2>
              <p className='text-dark-text-secondary text-sm uppercase tracking-widest'>
